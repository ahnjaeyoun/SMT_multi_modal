...
def predict(model, file, basename) -> dict[str, Any]:
    ...
    if results and results[0].boxes is not None:
        ...
        for index in range(len(boxes)):
            class_id = int(boxes.cls[index])      ## 학습기준 카테고리 0베이스 -> 1번부터 시작하기 위해 설정
            detections.append(
                {
                    "class_id": class_id,
                    "class_name": model.names[class_id],    ## model.names는 0베이스 -> 다시 -1
...
def draw_bbox(image_file, result, output_path, filename ,color=(0, 255, 0), thickness=2):
    ...
    for d in data['detections']:
        ...
        if d['class_id'] < 16:                 ## 결과값 id가 1부터 시작 -> 인덱스를 위해 -1
            label='정상'
            fill_color=(255,0,0)
            fill_line=(0,0,255)
        else: 
            label = CATEGORIES[d['class_id']]['name']
            print(f'그림그리는 라벨: {label}')
            fill_color=(0,0,255)
            fill_line=(255,0,0)
        # bbox 그리기
        cv2.rectangle(image, (x, y), (w, h), fill_line, thickness)
        ...
        if label:
            ...
            text_height= b-t
            pad=1
            draw.rectangle((x, y, x+text_width+pad, y+text_height+pad), fill=fill_color)
